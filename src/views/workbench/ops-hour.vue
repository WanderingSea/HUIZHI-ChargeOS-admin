<template>
  <div class="statistics-page">
    ...
    <div class="breadcrumb">
      ...
      ><span class="sep">></span><span>小时统计</span><span class="sep">></span
      ><span class="bc-active">小时统计列表</span>
    </div>
    ...
    <div class="filter-card">
      <div class="filter-row">
        ...
        <div class="filter-item">
          <label class="filter-label">小时范围</label>
          <el-select
    ...
    <div class="table-card">
      <div class="table-header">
        <div class="table-title-wrap">
          <span class="table-title">小时维度统计表</span>
...
export default {
  ...
  data() {
    ...
    for (let h = 0; h < 24; h++) {
      data.push({
        hour: h + ":00-" + (h + 1) + ":00",
        power: (Math.random() * 500 + 50).toFixed(2) * 1,
        chargeHours: (Math.random() * 20 + 2).toFixed(2) * 1,
        chargeCount: Math.floor(Math.random() * 40 + 5),
        elecFee: (Math.random() * 400 + 30).toFixed(2) * 1,
        serviceFee: (Math.random() * 40 + 3).toFixed(2) * 1,
        totalAmount: 0,
      });
      data[data.length - 1].totalAmount =
        (
          data[data.length - 1].elecFee + data[data.length - 1].serviceFee
        ).toFixed(2) * 1;
    }
    return {
      activeTopTab: "ops-hour",
